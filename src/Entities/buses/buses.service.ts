...
async createBus(bus : CreateBus){
    ...
    const {marque,puissance,matricule,picture,nbrePlaces,dateCreation, dateUpdate,status, agencyId, spaId } = bus;
    ...
    const newBus = this.busRepository.create({marque,puissance,matricule,nbrePlaces,dateCreation, dateUpdate,status,picture, agency, super_agent});
